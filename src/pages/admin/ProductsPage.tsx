  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 style={pageHeadingStyle}>Products</h1>
              ...
              <tr style={{ background: 'var(--surface-sunken)' }}>
                {['Product', 'Base Price', 'Discount', 'Status', 'Actions'].map((h) => (
                  <th
                    key={h}
                    style={thStyle}
                    className={`px-5 py-3 ${['Base Price', 'Discount'].includes(h) ? 'text-right' : 'text-left'}`}
                  >
                    {h}
                  </th>
                ))}
                ...
                >
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      {p.images[0] ? (
                        <img src={p.images[0].image_url} className="h-10 w-10 rounded-lg object-cover border border-[--border-subtle]" alt={p.name} />
                      ) : (
      ...
      <Dialog open={creating || !!editing} onClose={closeModal} className="relative z-50">
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          ...
          >
            <DialogTitle className="mb-4 font-bold" style={pageHeadingStyle}>{editing ? 'Edit Product' : 'New Product'}</DialogTitle>
